...
fty_driver_profile <- fty_drivers %>% 
  group_by(SEX,WHERE_FROM) %>% 
  summarize(proportion = n()/nrow(fty_drivers)) 

fty_driver_profile[which.max(fty_driver_profile$proportion),] #male locals make up 46.1% of crashes in Chicago where the driver failed to yield
...
ftc_driver_profile <- ftc_drivers %>% 
  group_by(SEX,WHERE_FROM) %>% 
  summarize(proportion = n()/nrow(ftc_drivers)) 

ftc_driver_profile[which.max(ftc_driver_profile$proportion),] #male locals make up 44% of crashes in Chicago where the driver was following too closely
...
iop_driver_profile <- iop_drivers %>% 
  group_by(SEX,WHERE_FROM) %>% 
  summarize(proportion = n()/nrow(iop_drivers)) 

iop_driver_profile[which.max(iop_driver_profile$proportion),] #male locals make up 41.1% of crashes in Chicago where the driver improperly overtook/passed someone
...
ftrs_driver_profile <- ftrs_drivers %>% 
  group_by(SEX,WHERE_FROM) %>% 
  summarize(proportion = n()/nrow(ftrs_drivers)) 

ftrs_driver_profile[which.max(ftrs_driver_profile$proportion),] #male locals make up 45.3% of crashes in Chicago where the driver failed to yield
...
ilu_driver_profile <- ilu_drivers %>% 
  group_by(SEX,WHERE_FROM) %>% 
  summarize(proportion = n()/nrow(ilu_drivers)) 

ilu_driver_profile[which.max(ilu_driver_profile$proportion),] #male locals make up 44.6% of crashes in Chicago where the driver used lane improperly
```
...
Where in the city are crashes of different severeties and causes happening?

```{r}
#find injury locations
fatal_injury_locations <- crashes %>% 
  drop_na(LATITUDE,LONGITUDE) %>% 
  filter(MOST_SEVERE_INJURY == "FATAL")

inc_injury_locations <- crashes %>% 
  drop_na(LATITUDE,LONGITUDE) %>% 
  filter(MOST_SEVERE_INJURY == "INCAPACITATING INJURY")

noinj_injury_locations <- crashes %>% 
  drop_na(LATITUDE,LONGITUDE) %>% 
  filter(MOST_SEVERE_INJURY == "NO INDICATION OF INJURY")

noninc_injury_locations <- crashes %>% 
  drop_na(LATITUDE,LONGITUDE) %>% 
  filter(MOST_SEVERE_INJURY == "NONINCAPACITATING INJURY")

rne_injury_locations <- crashes %>% 
  drop_na(LATITUDE,LONGITUDE) %>% 
  filter(MOST_SEVERE_INJURY == "REPORTED, NOT EVIDENT")
...
cause_locations <- crashes %>% 
  ...
  drop_na(PRIM_CONTRIBUTORY_CAUSE, LATITUDE, LONGITUDE) %>% 
  group_by(PRIM_CONTRIBUTORY_CAUSE) %>% 
...
#map fatal crashes
ggplot() + geom_polygon(shapefile_df, mapping=aes(x=long, y=lat, group=group)) + geom_path()  + geom_point(fatal_injury_locations,mapping=aes(x=LONGITUDE,y=LATITUDE, color="red"),size=1) + ggtitle("Fatal Crash Locations")

#map incapacitating crashes
ggplot() + geom_polygon(shapefile_df, mapping=aes(x=long, y=lat, group=group)) + geom_path()  + geom_point(fatal_injury_locations,mapping=aes(x=LONGITUDE,y=LATITUDE, color="red"),size=1) + ggtitle("Incapacitating Crash Locations")

require(sf)
require(mapview)


mapview(fatal_injury_locations, xcol = "LONGITUDE", ycol = "LATITUDE", crs = 4269, grid = FALSE, col.regions="red")

mapview(inc_injury_locations, xcol = "LONGITUDE", ycol = "LATITUDE", crs = 4269, grid = FALSE, col.regions="orange")

mapview(noninc_injury_locations, xcol = "LONGITUDE", ycol = "LATITUDE", crs = 4269, grid = FALSE, col.regions="yellow")

mapview(rne_injury_locations, xcol = "LONGITUDE", ycol = "LATITUDE", crs = 4269, grid = FALSE, col.regions="blue")

mapview(noinj_injury_locations, xcol = "LONGITUDE", ycol = "LATITUDE", crs = 4269, grid = FALSE, col.regions="green") 
```
