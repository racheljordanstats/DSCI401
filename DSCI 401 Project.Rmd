...
ggplot() + geom_polygon(shapefile_df, mapping=aes(x=long, y=lat, group=group)) + geom_path()  + geom_point(injury_locations,mapping=aes(x=LONGITUDE,y=LATITUDE, color=MOST_SEVERE_INJURY),size=0.5) + ggtitle("Crash Locations") #removed no injury crashes bc they were obscuring everything else. no patterns indicated by the map except less activity in far south side

#map of crashes by cause
ggplot() + geom_polygon(shapefile_df, mapping=aes(x=long, y=lat, group=group)) + geom_path()  + geom_point(cause_locations,mapping=aes(x=LONGITUDE,y=LATITUDE, color=PRIM_CONTRIBUTORY_CAUSE),size=0.5) + ggtitle("Crash Locations") #removed no injury crashes bc they were obscuring everything else. no patterns indicated by the map except less activity in far south side
```
